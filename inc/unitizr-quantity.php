...
{
    $wndnada = ( !isset( get_option('unitizr_options')['unitizr_wndnada_field']))
                ? false : true;
    
    if( !$wndnada ) {
       
        if( empty( $_POST['unitizr_numberof'] ) || 
                    $_POST['unitizr_numberof'] < 1 ){
 ...
 * @return array
 */ 
function unitizrs_render_priceperday_field( $title=null, $cart_item, $cart_item_key)
{
...
add_filter( 'woocommerce_cart_item_name', 'unitizrs_render_priceperday_field', 10, 3 ); 
